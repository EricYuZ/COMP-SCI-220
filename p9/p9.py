...
for key in movies:
    if 'Harry Potter' in key['title'] and 'David Yates' in key['directors']:
        rating.append(key['rating'])
        
median_hp_rating = median(rating)
...
for key in movies:
    if 'Avengers' in key['title']:
        if key['rating'] > highest_rating:
            highest_rating = key['rating']

highest_rated_avengers_movies = [key for key in movies if 'Avengers' in key['title'] \
                                 and key['rating'] == highest_rating]
...
genre_num = {}
for key1 in genre_buckets:
    for key2 in genre_buckets[key1]:
        if key1 not in genre_num:
            genre_num[key1] = 1
        else:
            genre_num[key1] = genre_num[key1] + 1
...
# The most popular genres is drama. The surprising is that the comedy is the second popular genres. 
...
for key1 in genre_buckets:
    for key2 in genre_buckets[key1]:
        if 'Akira Kurosawa' not in key2['directors']:
            continue
        if key1 not in kurosawa_genres:
            kurosawa_genres[key1] = 1
        else:
            kurosawa_genres[key1] = kurosawa_genres[key1] + 1
kurosawa_genres
...
for key1 in genre_buckets:
    for key2 in genre_buckets[key1]:
        if 'Christopher Nolan' not in key2['directors']:
            continue
        if key1 not in nolan_genres:
            nolan_genres[key1] = 1
        else:
            nolan_genres[key1] = nolan_genres[key1] + 1
plot_dict(nolan_genres, 'number of movies')
...
for key1 in year_buckets:
    for key2 in year_buckets[key1]:
        decade = year_to_decade(key1)
        if decade not in decade_mapping:
            decade_mapping[decade] = 1
        else:
            decade_mapping[decade] = decade_mapping[decade] + 1
...
for key1 in genre_buckets:
    list1 = []
    for key2 in genre_buckets[key1]:  
        list1.append(key2['rating'])
    median_genres[key1] = median(list1)
...
for key in genre_buckets['Animation']:
    if 'Shrek' in key['title']:
        shrek_inc.append(key)
        
shrek_inc = sorted(shrek_inc, key=lambda d:d['year'])
...
denzel_directors = []
times = {}
for key1 in bucketize(movies, 'cast')['Denzel Washington']:
    for key2 in key1['directors']:
        if key2 not in times:
            times[key2] = 1
        else:
            times[key2] = times[key2] + 1
denzel_directors = sorted(times, key=times.get, reverse = True)
