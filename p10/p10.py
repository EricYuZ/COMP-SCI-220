...
for i in files_in_data:
    if i.endswith('.json'):
        json_file_paths.append(os.path.join("data", i))
...
for i in files_in_data:
    if i.startswith('stars'):
        stars_paths.append(os.path.join("data", i))
...
def star_cell(row_idx, col_name, stars_rows, header=stars_header):
    ...
    if val == "":
        ...
        if col_name in ['Stellar Effective Temperature [K]', 'Stellar Radius [Solar Radius]',
                       'Stellar Mass [Solar mass]', 'Stellar Luminosity [log(Solar)]', 
                       'Stellar Surface Gravity [log10(cm/s**2)]', 'Stellar Age [Gyr]']:
            return float(val)
        elif col_name in ['Spectral Type', 'Name', 'Spectral Type']:
            return str(val)
...
# extract the values from stars_2_rows
...
for key in stars_1_dict:
    if not stars_1_dict[key].stellar_luminosity == None:
        total_lum = total_lum + stars_1_dict[key].stellar_luminosity
        num = num + 1
        
avg_lum_stars_1 = total_lum / num
...
for key in stars_2_dict:
    if not stars_2_dict[key].stellar_age == None:
        total_year = total_year + stars_2_dict[key].stellar_age
        num = num + 1
        
avg_age_stars_2 = total_year / num
...
for i in all_data:
    if i.startswith("stars"):
        stardict = get_stars(i)
        stars_dict.update(stardict)
# -
...
for i in stars_dict:
    if stars_dict[i].stellar_radius != None:
        if stars_dict[i].stellar_radius > largest:
            largest = stars_dict[i].stellar_radius 
            biggest_star = i
...
for i in stars_dict:
    if i.startswith('Kepler') and stars_dict[i].stellar_age != None:
        total_age = total_age + stars_dict[i].stellar_age
        num = num + 1
...
def planet_cell(row_idx, col_name, planets_rows, header=planets_header):
    ...
    if col_name in ["Orbital Period [days]", "Planet Radius [Earth Radius]",
        ...
        return float(val)
    if col_name in ["Discovery Method", 'Planet Name']:
        return str(val)
...
def get_planets(planet_file, mapping_file):
    ...
    # replace with your code
    planets_data = process_csv(os.path.join("data", planet_file)) 
...
list_planet = get_planets('planets_1.csv', 'mapping_1.json')
last_five_planets_1 = list_planet[-5:]
last_five_planets_1
# -
...
list_planet_2 = get_planets('planets_2.csv', 'mapping_2.json')

for i in list_planet_2:
    if i.controversial_flag:
        controversial_planets.append(i)
...
planet_list_4 = get_planets('planets_4.csv', 'mapping_4.json')

last_five_planets_4 = planet_list_4[-5:]
...
for i in files_in_data:
    if i.startswith('planets'):
        planets_file.append(i)
    if i.endswith('.json'):
        json_file.append(i)

for i in range(len(json_file)-1):
    planet = get_planets(planets_file[i], json_file[i])
    planets_list.extend(planet)
# -
...
for i in planets_list:
    if i.discovery_year == 2022:
        planets_disc_2022 = planets_disc_2022 + 1
...
for i in planets_list:
    if i.planet_name == 'TOI-2202 c':
        obj = i

toi_2022_c_star = stars_dict[obj.host_name]
...
for i in planets_list:
    if i.planet_radius == None:
        continue
    if stars_dict[i.host_name].stellar_radius == None:
        continue
    if stars_dict[i.host_name].stellar_radius > 10:
        total_radius = total_radius + i.planet_radius
        num = num + 1
...
for i in planets_list:
    if stars_dict[i.host_name].stellar_age == None:
        continue
    if youngest_age == None:
        youngest_age = stars_dict[i.host_name].stellar_age
    elif youngest_age > stars_dict[i.host_name].stellar_age:
        youngest_age = stars_dict[i.host_name].stellar_age
            
for i in planets_list:
    if stars_dict[i.host_name].stellar_age == youngest_age:
        youngest_star_planets.append(i)
